...
import phoneFrame from "@/imports/image-7.png";
  ...
  const runExport = useCallback(async () => {
    ...
    for (let i = 0; i < EXPORT_SCREENS.length; i++) {
      ...
      if (extra === "auth:splash") {
        ...
        screen = h(HomeScreen, { onOpenApps: noOp, onNav: noOp as any, onOpenNotifs: noOp });
...
function DesktopHomePanel({ onNav, onOpenApps }: { onNav: (t: TabName) => void; onOpenApps: () => void }) {
  const stats = [
    { label: "Organizations", sub: "Joined", value: "3", color: "#f5a623", icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#f5a623" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg> },
    { label: "Events", sub: "Registered", value: "5", color: "#38bdf8", icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#38bdf8" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> },
    { label: "Applications", sub: "Active", value: "3", color: "#10b981", icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="5" y="2" width="14" height="20" rx="2"/><line x1="9" y1="7" x2="15" y2="7"/><line x1="9" y1="11" x2="15" y2="11"/><line x1="9" y1="15" x2="12" y2="15"/></svg> },
    { label: "Merch", sub: "Saved", value: "2", color: "#a78bfa", icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg> },
  ];

  return (
    <div className="desktop-page flex flex-col h-full overflow-hidden">
      <div className="flex-shrink-0 flex items-end justify-between gap-4 mb-4">
        <div>
          <h1 className="font-black text-2xl text-white leading-tight">Welcome back, <span style={{ color: "#f5a623" }}>Juan</span>. 👋</h1>
          <p className="text-xs mt-1" style={{ color: "rgba(255,255,255,0.45)" }}>Discover organizations, events, and opportunities at Ateneo de Davao University.</p>
        </div>
        <button onClick={() => onNav("Organizations")} className="desktop-primary-btn flex-shrink-0">Browse Organizations</button>
      </div>

      <div className="grid grid-cols-4 gap-3 mb-4 flex-shrink-0">
        {stats.map(s => (
          <div key={s.label} className="desktop-stat-card">
            <div className="desktop-stat-icon" style={{ background: s.color + "18" }}>{s.icon}</div>
            <div className="min-w-0">
              <div className="flex items-baseline gap-2">
                <span className="font-black text-xl text-white leading-none">{s.value}</span>
                <span className="font-black text-xs text-white truncate">{s.label}</span>
              </div>
      ...
      <div className="flex-1 grid grid-cols-12 gap-3 min-h-0">
        <section className="col-span-7 desktop-panel flex flex-col min-h-0">
          <div className="desktop-panel-head">
            <div>
              <div className="font-black text-sm text-white">Featured Organizations</div>
              <div className="text-[10px] mt-0.5" style={{ color: "rgba(255,255,255,0.35)" }}>Organizations currently accepting members</div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto phone-scroll pr-1 flex flex-col gap-2">
            {ORGS.slice(0, 4).map(org => (
              <button key={org.name} onClick={() => onNav("Organizations")} className="desktop-org-row text-left">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-black text-xs leading-tight" style={{ color: "#0d1b3e" }}>{org.name}</div>
                  <div className="text-[10px] mt-0.5 truncate" style={{ color: "#64748b" }}>{org.category} · {org.desc}</div>
                </div>
                <StatusBadge status={org.status} />
              </button>
            ))}
        ...
        <div className="col-span-5 flex flex-col gap-3 min-h-0">
          <section className="desktop-panel flex-1 min-h-0 flex flex-col">
            <div className="desktop-panel-head">
              <div className="font-black text-sm text-white">Upcoming Events</div>
              <button onClick={() => onNav("Events")} className="desktop-link">See all</button>
            </div>
            <div className="flex-1 overflow-y-auto phone-scroll pr-1 flex flex-col gap-2">
              {EVENTS.slice(0, 4).map(ev => (
                <button key={ev.title} onClick={() => onNav("Events")} className="desktop-event-row text-left">
                  <div className="desktop-event-date">
                    <span>{ev.month}</span>
                    <strong>{ev.day}</strong>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="font-black text-xs leading-tight truncate" style={{ color: "#0d1b3e" }}>{ev.title}</div>
                    <div className="text-[10px] mt-1 truncate" style={{ color: "#64748b" }}>{ev.time} · {ev.location}</div>
                  </div>
                </button>
              ))}
            </div>
          ...
          <section className="desktop-panel flex-shrink-0">
            <div className="font-black text-xs text-white mb-2">Quick Access</div>
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => onOpenApps()} className="desktop-quick-btn">Applications</button>
              <button onClick={() => onNav("Merch")} className="desktop-quick-btn">Merch & Deals</button>
            </div>
          </section>
        </div>
      </div>
...
function DesktopOrgsPanel({ onSelect }: { onSelect: (org: typeof ORGS[0]) => void }) {
  const [category, setCategory] = useState("All Categories");
  const [status, setStatus] = useState("All Status");
  const [search, setSearch] = useState("");
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex-shrink-0 mb-5">
        <h2 className="font-black text-2xl text-white mb-1">Organizations</h2>
        <p className="text-sm" style={{ color: "rgba(255,255,255,0.45)" }}>Browse and apply to recognized student organizations at AdDU.</p>
      </div>

      {/* Search + filters row */}
      <div className="flex-shrink-0 flex gap-3 mb-4">
        <div className="flex-1 flex items-center gap-2 rounded-xl px-3 py-2.5" style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.1)" }}>
          <svg width="15" height="15" viewBox="0 0 24 24" fill="rgba(255,255,255,0.4)"><path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0016 9.5 6.5 6.5 0 109.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z" /></svg>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search organizations..." className="bg-transparent flex-1 text-sm text-white placeholder:text-white/30 outline-none" />
        </div>
        <select value={category} onChange={e => setCategory(e.target.value)} className="rounded-xl px-3 py-2.5 text-xs font-semibold outline-none" style={{ background: "#0e1e38", border: "1px solid rgba(255,255,255,0.12)", color: category !== "All Categories" ? "#f5a623" : "rgba(255,255,255,0.7)" }}>
          {ORG_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
        </select>
        <select value={status} onChange={e => setStatus(e.target.value)} className="rounded-xl px-3 py-2.5 text-xs font-semibold outline-none" style={{ background: "#0e1e38", border: "1px solid rgba(255,255,255,0.12)", color: status !== "All Status" ? "#f5a623" : "rgba(255,255,255,0.7)" }}>
      ...
      <div className="flex-1 overflow-y-auto phone-scroll">
        <div className="grid grid-cols-3 gap-3 pb-4">
          {filtered.map(org => (
            <button key={org.name} onClick={() => onSelect(org)} className="rounded-2xl p-4 flex flex-col gap-3 text-left" style={{ background: "#0e1c35", border: "1px solid rgba(255,255,255,0.07)" }}>
              <div className="flex items-start justify-between">
                <OrgLogo name={org.name} size={52} />
                <StatusBadge status={org.status} />
              </div>
              <div>
                <div className="text-white font-black text-sm leading-tight mb-1">{org.name}</div>
                <div className="text-xs leading-relaxed" style={{ color: "rgba(255,255,255,0.5)" }}>{org.desc}</div>
              </div>
              <div className="flex items-center justify-between mt-auto">
                <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: org.color + "22", color: org.color }}>{org.category}</span>
                <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M6 4l4 4-4 4" stroke="rgba(255,255,255,0.3)" strokeWidth="2" strokeLinecap="round"/></svg>
              </div>
            </button>
          ))}
          {filtered.length === 0 && (
...
function DesktopEventsPanel() {
  const [typeFilter, setTypeFilter] = useState("All Events");
  const upcoming = EVENTS.filter(e => typeFilter === "All Events" || e.type === typeFilter);

  return (
    <div className="desktop-page flex flex-col h-full overflow-hidden">
      <div className="flex-shrink-0 flex items-end justify-between gap-4 mb-4">
        <div>
          <h2 className="font-black text-2xl text-white leading-tight">Events</h2>
          <p className="text-xs mt-1" style={{ color: "rgba(255,255,255,0.45)" }}>Stay up to date with organization and campus events at AdDU.</p>
        </div>
        <div className="flex gap-1.5 flex-wrap justify-end">
          {["All Events", "Org", "Academic", "Arts", "Tech"].map(f => (
            <button key={f} onClick={() => setTypeFilter(f)} className="desktop-filter-chip" data-active={typeFilter === f ? "true" : "false"}>{f}</button>
          ))}
        </div>
      </div>

      <div className="flex-1 grid grid-cols-12 gap-3 min-h-0">
        <section className="col-span-8 desktop-panel flex flex-col min-h-0">
          <div className="desktop-panel-head">
            <div>
              <div className="font-black text-sm text-white">Upcoming Events</div>
              <div className="text-[10px] mt-0.5" style={{ color: "rgba(255,255,255,0.35)" }}>{upcoming.length} events shown</div>
            </div>
          </div>
          <div className="flex-1 overflow-y-auto phone-scroll pr-1 flex flex-col gap-2">
            {upcoming.map(ev => (
              <button key={ev.title} className="desktop-event-list-row text-left">
                <div className="desktop-event-list-date">
                  <span>{ev.month}</span>
                  <strong>{ev.day}</strong>
                  <small>{ev.dayName}</small>
                </div>
                <div className="flex-1 min-w-0 py-0.5">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="font-black text-sm leading-tight truncate" style={{ color: "#0d1b3e" }}>{ev.title}</div>
                    <span className="desktop-type-pill flex-shrink-0" style={{ background: ev.typeColor + "22", color: ev.typeColor }}>{ev.type}</span>
                  </div>
                  <div className="text-[10px] mt-1" style={{ color: "#64748b" }}>{ev.time}</div>
                  <div className="text-[10px] mt-0.5 truncate" style={{ color: "#64748b" }}>{ev.location}</div>
                </div>
                <svg width="14" height="14" viewBox="0 0 16 16" fill="none" className="flex-shrink-0"><path d="M6 4l4 4-4 4" stroke="#94a3b8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
              </button>
            ))}

            {typeFilter === "All Events" && (
              <>
                <div className="text-[10px] font-black uppercase tracking-wider mt-2 mb-0.5" style={{ color: "rgba(255,255,255,0.35)" }}>Past Events</div>
                {PAST_EVENTS.map(ev => (
                  <div key={ev.title} className="desktop-event-list-row opacity-70">
                    <div className="desktop-event-list-date" style={{ background: "#334155" }}>
                      <span>{ev.month}</span><strong>{ev.day}</strong><small>{ev.dayName}</small>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="font-black text-sm truncate" style={{ color: "#475569" }}>{ev.title}</div>
                      <div className="text-[10px] mt-1 truncate" style={{ color: "#94a3b8" }}>{ev.time} · {ev.location}</div>
                    </div>
                  </div>
                ))}
              </>
        ...
        <aside className="col-span-4 flex flex-col gap-3 min-h-0">
          <section className="desktop-panel flex-shrink-0">
            <div className="flex items-center justify-between mb-3">
              <div className="font-black text-sm text-white">September 2026</div>
              <div className="text-[10px]" style={{ color: "rgba(255,255,255,0.35)" }}>Calendar</div>
            </div>
            <div className="grid grid-cols-7 gap-1 mb-1">
              {["S","M","T","W","T","F","S"].map((d, i) => <div key={i} className="text-center text-[9px] font-bold py-1" style={{ color: "rgba(255,255,255,0.3)" }}>{d}</div>)}
            </div>
            <div className="grid grid-cols-7 gap-1">
              ...
              {[...Array(30)].map((_, i) => {
                ...
                const hasEvent = [5,12,18,25].includes(day);
                const isToday = day === 7;
                return <div key={day} className="desktop-cal-day" data-today={isToday ? "true" : "false"} data-event={hasEvent ? "true" : "false"}>{day}</div>;
              })}
            </div>
          </section>

          <section className="desktop-panel flex-1 min-h-0 flex flex-col">
            <div className="font-black text-sm text-white mb-2">This Month</div>
            <div className="flex-1 overflow-y-auto phone-scroll pr-1">
              {EVENTS.filter(e => e.month === "SEP").map(ev => (
                <div key={ev.title} className="flex gap-2 py-2.5" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                  <div className="w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0" style={{ background: ev.typeColor }} />
                  <div className="min-w-0">
                    <div className="text-xs font-bold text-white leading-tight truncate">{ev.title}</div>
                    <div className="text-[10px] mt-1" style={{ color: "rgba(255,255,255,0.4)" }}>Sep {ev.day} · {ev.time.split("–")[0].trim()}</div>
                  </div>
                </div>
              ))}
            </div>
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex-shrink-0 mb-5">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-black text-2xl text-white mb-1">Marketplace</h2>
            <p className="text-sm" style={{ color: "rgba(255,255,255,0.45)" }}>Official org merch and trusted local sellers for AdDU students.</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex rounded-xl overflow-hidden" style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.08)" }}>
              {(["merch", "sellers"] as const).map(v => (
                <button key={v} onClick={() => setView(v)} className="px-5 py-2 text-xs font-black transition-all" style={{ background: view === v ? "#f5a623" : "transparent", color: view === v ? "#020b18" : "rgba(255,255,255,0.5)", borderRadius: view === v ? 10 : 0 }}>
                  {v === "merch" ? "Org Merch" : "Local Sellers"}
                </button>
              ))}
            </div>
            <button className="relative w-10 h-10 rounded-full flex items-center justify-center" style={{ background: "rgba(255,255,255,0.08)" }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.75)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>
              {cart.length > 0 && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full flex items-center justify-center font-black" style={{ background: "#ef4444", color: "#fff", fontSize: 9 }}>{cart.length}</span>}
            </button>
          </div>
...
function DesktopProfilePanel({ onOpenApps }: { onOpenApps: () => void }) {
  const menuGroup1 = [
    { label: "My Applications", onClick: onOpenApps, icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.75)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="5" y="2" width="14" height="20" rx="2"/><line x1="9" y1="7" x2="15" y2="7"/><line x1="9" y1="11" x2="15" y2="11"/><line x1="9" y1="15" x2="12" y2="15"/></svg> },
    { label: "Saved Organizations", onClick: () => {}, icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.75)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg> },
    { label: "Registered Events", onClick: () => {}, icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.75)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> },
  ];
  const menuGroup2 = [
    { label: "Notification Settings", onClick: () => {}, icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.75)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></svg> },
    { label: "Help & Support", onClick: () => {}, icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.75)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 015.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> },
    { label: "Log Out", onClick: () => {}, danger: true, icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.75)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg> },
  ];
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 grid grid-cols-3 gap-6 min-h-0 overflow-y-auto phone-scroll pb-4">
        {/* Left: Avatar card */}
        <div className="col-span-1">
          <div className="rounded-2xl p-6 flex flex-col items-center text-center" style={{ background: "#0e1e38", border: "1px solid rgba(255,255,255,0.07)" }}>
            <div className="relative mb-4">
              <div className="rounded-full flex items-center justify-center" style={{ width: 110, height: 110, background: "#162347", border: "3px solid rgba(255,255,255,0.1)" }}>
                <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
              </div>
              <button className="absolute bottom-0 right-0 w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "#162347", border: "2px solid rgba(255,255,255,0.15)" }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.8)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/></svg>
              </button>
            </div>
            <div className="text-white font-black text-xl mb-1">Juan Dela Cruz</div>
            <div className="text-sm mb-0.5" style={{ color: "rgba(255,255,255,0.5)" }}>BS Information Technology</div>
            <div className="text-sm mb-3" style={{ color: "rgba(255,255,255,0.5)" }}>3rd Year</div>
            <div className="text-xs px-3 py-1 rounded-full mb-4" style={{ background: "rgba(56,189,248,0.12)", color: "#38bdf8" }}>juan.delacruz@addu.edu.ph</div>

            <div className="grid grid-cols-3 gap-2 w-full pt-4" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
              {[{ label: "Orgs", val: "3" }, { label: "Events", val: "5" }, { label: "Apps", val: "3" }].map(s => (
                <div key={s.label} className="text-center">
                  <div className="font-black text-lg text-white">{s.val}</div>
        ...
        <div className="col-span-2 flex flex-col gap-4">
          <div className="rounded-2xl overflow-hidden" style={{ background: "#0e1e38", border: "1px solid rgba(255,255,255,0.07)" }}>
            <div className="px-5 py-3 text-xs font-black" style={{ color: "rgba(255,255,255,0.35)", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>ACCOUNT</div>
            {menuGroup1.map((item, i) => (
              <button key={item.label} onClick={item.onClick} className="w-full flex items-center gap-4 px-5 py-4 text-left hover:bg-white/5 transition-colors" style={{ borderBottom: i < menuGroup1.length - 1 ? "1px solid rgba(255,255,255,0.06)" : "none" }}>
                {item.icon}
                <span className="flex-1 text-white font-semibold text-sm">{item.label}</span>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.35)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18l6-6-6-6"/></svg>
              </button>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: "#0e1e38", border: "1px solid rgba(255,255,255,0.07)" }}>
            <div className="px-5 py-3 text-xs font-black" style={{ color: "rgba(255,255,255,0.35)", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>SETTINGS</div>
            {menuGroup2.map((item: { label: string; onClick: () => void; danger?: boolean; icon: React.ReactNode }, i: number) => (
              <button key={item.label} onClick={item.onClick} className="w-full flex items-center gap-4 px-5 py-4 text-left hover:bg-white/5 transition-colors" style={{ borderBottom: i < menuGroup2.length - 1 ? "1px solid rgba(255,255,255,0.06)" : "none" }}>
                {item.icon}
                <span className="flex-1 font-semibold text-sm" style={{ color: item.danger ? "#ef4444" : "#fff" }}>{item.label}</span>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.35)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18l6-6-6-6"/></svg>
              </button>
  ...
  if (!authed) {
    return (
      <div className="w-full h-full flex items-center justify-center" style={{ background: BG }}>
        <div className="w-full max-w-md flex flex-col" style={{ fontFamily: "'Nunito', sans-serif", color: "white" }}>
    ...
    <div className="w-full h-full flex" style={{ background: BG, fontFamily: "'Nunito', sans-serif", color: "#fff" }}>
      ...
      <aside className="flex-shrink-0 flex flex-col" style={{ width: 205, background: "#050e1c", borderRight: "1px solid rgba(255,255,255,0.07)" }}>
        ...
        <div className="flex-shrink-0 px-4 py-4" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="flex items-center gap-3 p-3 rounded-xl" style={{ background: "rgba(255,255,255,0.04)" }}>
            ...
            <div className="flex-1 min-w-0">
              <div className="text-white font-black text-xs truncate">Juan Dela Cruz</div>
              <div className="text-xs truncate" style={{ color: "rgba(255,255,255,0.35)", fontSize: 9 }}>3rd Year · BS IT</div>
            </div>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-shrink-0 flex items-center justify-between px-5 py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)", background: "#030d1b" }}>
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setShowNotifs(!showNotifs)} className="relative w-9 h-9 rounded-full flex items-center justify-center" style={{ background: "rgba(255,255,255,0.07)" }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="rgba(255,255,255,0.7)"><path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"/></svg>
              {notifCount > 0 && <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full" style={{ background: "#ef4444" }} />}
            </button>
          </div>
        </div>
            ...
            {!showApps && activeTab === "Organizations" && (
              applyingOrg
                ...
                : selectedOrg
                  ...
                  : <DesktopOrgsPanel onSelect={setSelectedOrg} />
            )}
            {!showApps && activeTab === "Events" && <DesktopEventsPanel />}
            {!showApps && activeTab === "Merch" && <DesktopMerchPanel />}
            {!showApps && activeTab === "Profile" && <DesktopProfilePanel onOpenApps={() => setShowApps(true)} />}
          </div>
          ...
          {showNotifs && (
            <div className="w-80 flex-shrink-0 flex flex-col" style={{ borderLeft: "1px solid rgba(255,255,255,0.07)", background: "#050e1c" }}>
              <div className="flex items-center justify-between px-5 py-4 flex-shrink-0" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                ...
                <div className="flex items-center gap-3">
                  <button className="text-xs font-semibold" style={{ color: "#38bdf8" }}>Mark all read</button>
                  <button onClick={() => setShowNotifs(false)} className="w-7 h-7 rounded-full flex items-center justify-center" style={{ background: "rgba(255,255,255,0.07)" }}>
                    <svg width="12" height="12" viewBox="0 0 16 16" fill="none"><path d="M4 4l8 8M12 4l-8 8" stroke="rgba(255,255,255,0.6)" strokeWidth="1.8" strokeLinecap="round"/></svg>
                  </button>
                </div>
              </div>
              <div className="flex-1 overflow-y-auto phone-scroll px-4 py-3 flex flex-col gap-2">
                {NOTIFS.map(n => (
                  <div key={n.title} className="rounded-xl p-3 flex gap-3" style={{ background: "#0e1e38", border: n.unread ? "1px solid rgba(245,166,35,0.2)" : "1px solid rgba(255,255,255,0.07)" }}>
                    <div className="flex-shrink-0 w-2 h-2 rounded-full mt-1.5" style={{ background: n.unread ? "#f5a623" : "transparent" }} />
                    <div className="flex-1 min-w-0">
